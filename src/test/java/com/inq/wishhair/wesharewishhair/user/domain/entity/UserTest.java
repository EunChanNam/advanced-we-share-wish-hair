...
class UserTest {
	...
	private final PasswordEncoder passwordEncoder = new PasswordEncoderStub();
	...
	class of {
		...
		void failByPassword(String password) {
			//when
			ThrowingCallable when = () -> Password.encrypt(password, passwordEncoder);
	...
	class updateFaceShape {
		...
		void fail() {
			...
			//when
			ThrowingCallable when = () -> user.updateFaceShape(new FaceShape(tag));
